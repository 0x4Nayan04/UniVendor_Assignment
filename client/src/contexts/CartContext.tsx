    ...
    if (isAuthenticated && !isAuthLoading && localCart.cart && localCart.cart.items && localCart.cart.items.length > 0) {
      const mergeLocalCartWithServer = async () => {
        try {
          ...
          console.log('Local cart items:', localCart.cart.items);
          ...
          for (const item of localCart.cart.items) {
            try {
              ...
              console.log('Adding item to server cart:', serverItem);
              
              await serverCart.addToCart(serverItem);
              mergedItemsCount++;
            } catch (itemError) {
              console.error('Error adding individual item to server cart:', itemError);
          ...
          // Refresh server cart to show merged items
          ...
          toast({
            ...
            description: "Some items from your guest cart couldn't be synced. Please check your cart.",
            variant: "destructive",
          });
        }
      };
  ...
  const addToCart = (item: any) => {
    try {
      if (isAuthenticated && serverCart) {
        ...
        console.log('Adding to server cart:', item);
        serverCart.addToCart(item);
      } else if (localCart) {
        // Use local cart for guest users - convert format if needed
